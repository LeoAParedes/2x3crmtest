...
export const CashiersPanel = () => {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setLoading(true)
      try {
        const listResponse = await fetch('/api/admin/cashiers')
        const listPayload = (await listResponse.json()) as {
          success?: boolean
          cashiers?: CashierRow[]
        }
        if (listResponse.ok && listPayload.success) {
          setCashiers(listPayload.cashiers || [])
        }
      } finally {
        setLoading(false)
      }
  ...
  return (
    <div className='space-y-6'>
      <section className='rounded-2xl border border-slate-200 bg-white p-6 shadow-sm'>
        <h2 className='text-lg font-semibold text-slate-950'>Alta de cajeros</h2>
